  ...
  el.innerHTML = `
    ...
    <div class="toolbar">
      ...
      <div style="display:flex; gap:8px">
        <button class="btn btn-ghost btn-sm" onclick="abrirModalFolhaCaixa()"><span class="icon">📄</span> Folha de Caixa Diária (PDF)</button>
...
async function exportarFolhaCaixaPdf(dataStr) {
  try {
    ...
    const dados = await api('GET', `/api/relatorios/folha-caixa-diaria?data=${dataStr}`);
    ...
    `;
    exportarHtmlParaPdf(html, `folha-caixa-${dataStr}.pdf`);
  } catch (err) {
    toast(err.message, 'error');
  }
}
...
function abrirAssinaturaContrato(id) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      // 1. Guardar assinaturas na base de dados
      toast('A registar assinaturas…');
      ...
      // 2. Gerar e submeter automaticamente o PDF assinado
      let docTexto = texto;
      if (sigFormando) {
      ...
      if (window.html2pdf) {
        toast('A gerar e submeter PDF assinado…');
        ...
        try {
          ...
          const pdfBase64 = await window.html2pdf().set(opt).from(tempDiv).output('datauristring');
          tempDiv.remove();

          await api('POST', `/api/contratos/${id}/pdf-assinado`, {
            pdfBase64,
            filename: `contrato-${id}-assinado.pdf`
          });
        } catch (pdfErr) {
          console.error('Erro ao gerar/submeter PDF do contrato:', pdfErr);
